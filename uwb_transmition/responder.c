...
#define ACCUM_DATA_LEN (3 * 2 * (32) + 1)
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (status_reg & DWT_INT_RXFCG_BIT_MASK)
                {
                    ...
                    if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                    {
                        ...
                        deca_uart_transmit(dist_str, sizeof(dist_str));

                        /* Read accumulator. See NOTES 2 and 6. */
                        uint16_t fp_int = rx_diag.ipatovFpIndex >> 6;
                        //uint16_t fp_int = rx_diag.ipatovF1  >> 6;
                        //dwt_readaccdata(accum_data, ACCUM_DATA_LEN, (fp_int - 2));

                        for(int i = 0; i < 31 ; i ++){
                          dwt_readaccdata(accum_data, 32*6+1, 0+32*i);
                          for(int j = 0; j < 32; j++){
                            cidx = 6 * j;
                            real = (int32_t)accum_data[cidx+3] << 16 | (int32_t)accum_data[cidx+2] << 8 | (int32_t)accum_data[cidx+1];
                            imag = (int32_t)accum_data[cidx+6] << 16 | (int32_t)accum_data[cidx+5] << 8 | (int32_t)accum_data[cidx+4];
                            if (real & 0x020000)  // MSB of 18 bit value is 1
                              real |= 0xfffc0000;
                            if (imag & 0x020000)  // MSB of 18 bit value is 1
                              imag |= 0xfffc0000;
                          //printf("%02x", accum_data[i]);
                          double CIRV = sqrt((float)(real*real+imag*imag));
                          //printf("%d ", real);
                          sprintf(cir_str, "%.2f, ", CIRV);
                          deca_uart_transmit(cir_str, sizeof(cir_str));
                          printf("%f", CIRV);
                          }
                        }
                        //dwt_readaccdata(accum_data, 24*6+1, 992);
                        //for(int j = 0; j < 24; j++){
                        //        cidx = 6 * j;
                        //        real = (int32_t)accum_data[cidx+3] << 16 | (int32_t)accum_data[cidx+2] << 8 | (int32_t)accum_data[cidx+1];
                        //        imag = (int32_t)accum_data[cidx+6] << 16 | (int32_t)accum_data[cidx+5] << 8 | (int32_t)accum_data[cidx+4];
                        ...
                        //      double CIRV = sqrt((float)(real*real+imag*imag));
                        //      //printf("%f ", CIRV);
                        //      sprintf(cir_str, "%.1f, ", CIRV);
                        //      deca_uart_transmit(cir_str, sizeof(cir_str));
                        //}
                        ////sprintf(cir_str, " EOF\n");
                        //deca_uart_transmit(cir_str, sizeof(cir_str));
                        //sprintf(dist_str, "[%d,%d,%d]", cluster_addr, resp_addr, (int32_t)(distance*100));
